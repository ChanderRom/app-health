...
import { AppHealthAddApllicationsContextEvent } from '../events/app-health-add-apllications-context.event';

@Injectable()
export class AppHealthUpdateApllicationsService
{
    constructor(
    ...
    ): Promise<void>
    {
        ...
        const apllicationsRegister = this.publisher.mergeObjectContext(
            new AppHealthAddApllicationsContextEvent(apllications),
        );

        apllicationsRegister.updated(); // apply event to model events
        apllicationsRegister.commit(); // commit all events of model
    }
}