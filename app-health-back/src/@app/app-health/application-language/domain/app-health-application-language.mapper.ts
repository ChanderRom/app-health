...
export class AppHealthApplicationLanguageMapper implements IMapper
{
     ...
     * @param applitacionLanguages
     */
    mapModelsToAggregates(applitacionLanguages: LiteralObject[], cQMetadata?: CQMetadata): AppHealthApplicationLanguage[]
    {
        if (!Array.isArray(applitacionLanguages)) return;

        return applitacionLanguages.map(applicationLanguage => this.makeAggregate(applicationLanguage, cQMetadata));
    }
     ...
     * @param applitacionLanguages
     */
    mapAggregatesToResponses(applitacionLanguages: AppHealthApplicationLanguage[]): AppHealthApplicationLanguageResponse[]
    {
        if (!Array.isArray(applitacionLanguages)) return;

        return applitacionLanguages.map(applicationLanguage => this.makeResponse(applicationLanguage));
    }
